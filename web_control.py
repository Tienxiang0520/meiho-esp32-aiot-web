...
def control():
    ...
    try:
        ...
        print(
            f"✅ 已透過 MQTT 發送新指令通知到主題：{NOTIFY_TOPIC}，內容：{notification_payload}"
        )
    except Exception as e:
...
def update_command_status():
    ...
    if "resolved_command" in data:
        latest_command["resolved_command"] = data["resolved_command"]

    if "error" in data:
        latest_command["error"] = data["error"]
...
def clear_command():
    ...
    latest_command["resolved_command"] = None
    latest_command["error"] = None
