...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (msg == "LED_ON") {
    ...
    display.display();
    status_msg = "LED_ON done"; // <--- 設定狀態訊息
  } else if (msg == "LED_OFF") {
    digitalWrite(ledPin, LOW);
    display.setCursor(0, 32);
    display.println(" LED OFF");
    display.display();
    status_msg = "LED_OFF done"; // <--- 設定狀態訊息
  }

  // 📢 [優化新增] 執行完畢後，立即發布狀態到新主題
  if (status_msg != "") {
    client.publish("/esp32/status", status_msg.c_str()); // <--- 發布到 /esp32/status
